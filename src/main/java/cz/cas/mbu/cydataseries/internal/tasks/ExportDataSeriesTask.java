...
public class ExportDataSeriesTask extends AbstractValidatedTask {
	...
	public ExportDataSeriesTask(DataSeriesManager dataSeriesManager,
	...
	protected ValidationState getValidationState(StringBuilder messageBuilder) {
		...
		if (!outputFile.getParentFile().exists()) {
			messageBuilder
					.append("The parent directory of the output file has to exist.");
			return ValidationState.INVALID;
		}
		if (outputFile.exists()) {
			messageBuilder.append("Are you sure you want to overwrite file '"
					+ outputFile.toString() + "'?");
	...
	public void run(TaskMonitor taskMonitor) throws Exception {
		...
		DataSeriesStorageProvider storageProvider = dataSeriesStorageManager
				.getStorageProvider(selectedSeries.getClass());
		if (storageProvider == null) {
			throw new DataSeriesException(
					"Could not export - no storage provider found for class "
							+ selectedSeries.getClass().getName());
		} else {
